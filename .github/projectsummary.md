...
- **Development Mode**: Triggered via GPIO13 (low) or remotely via config (hardware pin overrides remote config). Mode switches trigger a soft reset. Simulates the full lifecycle (Wake -> Transmit -> Receive -> Multi-screen Display (including total TX/RX power-on time tracking) -> Simulated Sleep with display off matching the configured sleep interval) continuously without entering actual ESP32 deep sleep. RTC memory is validated across deep sleeps and software resets (using RTC_NOINIT_ATTR) via a Magic Word and initialized automatically on cold boots. Base timestamp defaults to Jan 1 2024 before any remote sync.
