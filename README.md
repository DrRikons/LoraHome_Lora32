...
-  **Logging & MQTT**: The gateway uses native ESP-IDF logging (`esp_log.h`) with standard severity levels and automatically prepends real-world formatted timestamps when the system clock is synchronized. Log writes are dynamically redirected and queued via a background FreeRTOS task to append seamlessly to `/gateway_log.txt` on the SD card without causing any latency during critical radio interrupts. Sent MQTT payloads are also automatically injected with an ISO-8601 formatted `timestamp` property once the NTP validates.
