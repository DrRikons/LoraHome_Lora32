...
- **Development Mode**: Triggered via GPIO13 (low) or remotely via config (hardware pin overrides remote config). Mode switches trigger a soft reset. Simulates the full lifecycle (Wake -> Transmit -> Receive -> Multi-screen Display (including TX/RX Time-on-Air tracking) -> Simulated Sleep with display off) continuously without entering actual ESP32 deep sleep. RTC memory is validated across deep sleeps using a Magic Word. Base timestamp defaults to Jan 1 2024 before any remote sync.
