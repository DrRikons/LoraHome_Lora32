...
`CONFIG:<sleepInterval>,<version>[,<devMode>]`
...
To maximize LoRa time-on-air efficiency and save battery, the sensor node transmits telemetry data as a tightly packed, 27-byte binary C++ `struct`. The gateway must decode this binary format using the identical struct blueprint.
...
```
*(Note: Any floating point values like temperature are multiplied by 100 before transmission. The Gateway should divide them by 100.0 upon receipt to restore the decimal values).*
